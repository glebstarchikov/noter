...
export async function POST(req: Request) {
  ...
  const { data: meeting } = await supabase
    .from('meetings')
    .select('title, transcript, summary, action_items, key_decisions, topics, follow_ups')
    .eq('id', meetingId)
    .single()
  ...
  if (meeting.action_items && Array.isArray(meeting.action_items) && meeting.action_items.length > 0) {
    ...
    for (const item of meeting.action_items as Array<{ task: string; owner: string | null; done: boolean }>) {
      context += `- [${item.done ? 'x' : ' '}] ${item.task}${item.owner ? ` (Owner: ${item.owner})` : ''}\n`
    }
    context += '\n'
  }

  if (meeting.key_decisions && Array.isArray(meeting.key_decisions) && meeting.key_decisions.length > 0) {
  ...
  if (meeting.follow_ups && Array.isArray(meeting.follow_ups) && meeting.follow_ups.length > 0) {
    context += `## Follow-ups\n`
